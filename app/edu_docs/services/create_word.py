...
def createword(theme, subtopics, texts, university, type_works, author_name, group_name, teacher_name, language_of_work, cover_page_data):
    """
    ...
    if language_of_work == "1":
        ...
        if cover_page_data == "1" or cover_page_data == "3":
            ...
            if type_works == "Студенттин өз алдынча иши":
            ...
            type_works_title = doc.add_paragraph(type_works)
            ...
            theme_title = doc.add_paragraph("Тема: " + theme)
            ...
            if type_works == "Студенттин өз алдынча иши":
                ...
                if len(theme) < 59:
                    ...
                    for _ in range(4):
                        doc.add_paragraph()
                elif 59 <= len(theme) < 113:
                    # Добавляем пустые строки
                    for _ in range(3):
                        doc.add_paragraph()
                elif 114 <= len(theme) < 179:
                    # Добавляем пустые строки
                    for _ in range(3):
                        doc.add_paragraph()
                elif 180 <= len(theme):
        ...
        studai_work_path = os.path.join(desktop_path, 'Studai работы')
        ...
        # Обновляем содержание и закрываем документ
        ...
        # doc.save(sanitized_theme + ".docx")
        #
        # # Получаем текущий каталог
        # current_directory = os.path.dirname(os.path.abspath(__file__))
        #
        # # Путь к файлу
        # file_path = os.path.join(current_directory, sanitized_theme + ".docx")
        #
        # # Создаем объект Word и открываем документ
        # word = win32com.client.DispatchEx("Word.Application")
        # doc = word.Documents.Open(file_path)
    ...
    if language_of_work == "2":
        ...
        if cover_page_data == "1" or cover_page_data == "3":
            ...
            if type_works == "Самостоятельная работа студента":
                # Добавляем пустые строки
                for _ in range(3):
                    doc.add_paragraph()
            else:
            ...
            # Добавляем название темы
            theme_title = doc.add_paragraph("На тему: " + theme)
            theme_title.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
            theme_title.runs[0].font.name = "Times New Roman"
            theme_title.runs[0].font.size = Pt(14)
            theme_title.paragraph_format.line_spacing = 1.15
    ...
    if language_of_work == "3":
        ...
        if cover_page_data == "1" or cover_page_data == "3":
            ...
            if type_works == "Student's independent work":
                # Добавляем пустые строки
                for _ in range(3):
                    doc.add_paragraph()
            else:
            ...
            # Добавляем название темы
            theme_title = doc.add_paragraph("On the topic: " + theme)
            theme_title.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
            theme_title.runs[0].font.name = "Times New Roman"
            theme_title.runs[0].font.size = Pt(14)
            theme_title.paragraph_format.line_spacing = 1.15
